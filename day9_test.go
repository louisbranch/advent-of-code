...
func TestDecompress(t *testing.T) {
	...
	for _, tt := range testCases {
		...
		if tt.result != got {
			t.Errorf("Decompress(%s) %s != %s", tt.input, tt.result, got)
		}
	}
}

func TestRecursiveDecompress(t *testing.T) {
	testCases := []struct {
		input  string
		length int
	}{
	...
	for _, tt := range testCases {
		got := RecursiveDecompress(tt.input)
		if tt.length != got {
			t.Errorf("RecursiveDecompress(%s) %d != %d", tt.input, tt.length, got)
		}
...
/*
func TestBulkDecompress(t *testing.T) {
	...
	v1, v2 := BulkDecompress(input)

	if v1 != 150914 {
		t.Errorf("BulkDecompress() V1 %d", v1)
	}

	if v2 != 11052855125 {
		t.Errorf("BulkDecompress() V2 %d", v2)
	}
}
*/
